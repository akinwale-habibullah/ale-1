...
async function approvalModule(loan, userId, status, userPosition, FIRSTLEVELLOANAPPROVER, SECONDLEVELLOANAPPROVER) {
  ...
  if (userPosition == FIRSTLEVELLOANAPPROVER && status == 'approved') {
    const updatedLoan = await loan.update({ approver1: userId })
    if (updatedLoan.approver1 == userId) {
      return {
        success: true,
  ...
  if (userPosition == SECONDLEVELLOANAPPROVER && status == 'approved') {
    const updatedLoan = await loan.update({ approver2: userId, status: approved })
    if (updatedLoan.approver2 == userId) {
      return {
        success: true,
  ...
  if (userPosition == FIRSTLEVELLOANAPPROVER && status == 'rejected') {
    const updatedLoan = await loan.update({ rejecter1: userId })
    if (updatedLoan.rejecter1 == userId) {
      return {
        success: true,
  ...
  if (userPosition == SECONDLEVELLOANAPPROVER && status == 'rejected') {
    const updatedLoan = await loan.update({ rejecter2: userId, status: rejected })
    if (updatedLoan.rejecter2 == userId) {
      return {
        success: true,
...
exports.handler = async function approveLoans(req, res, next) {
  ...
  try {
    ...
    const loanCheck = await validateLoanRequest(loan.debtorId, loan.guarantor1, loan.guarantor2, loan.loanAmount);
