...
exports.handler = async function postLoans(req, res, next) {
  ...
  const { loanAmount, tenure, guarantor1Id, guarantor2Id, comments, type } = req.body;
  ...
  try {
    ...
    const newLoan = await Loan.create({ debtorId: userId, loanAmount, tenure, guarantor1Id, guarantor2Id, comments, type, status: 'pending' })
    res.json({
      success: true,
