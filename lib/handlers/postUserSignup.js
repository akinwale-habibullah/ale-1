...
exports.handler = function postUserSignup(req, res, next) {
    const { firstName, lastName, email, userName, password, joinDate, memberId, nextSavingsDate, nextSavingsAmount, position, phone, status, isGuarantor} = req.body;
    const hashPassword = generateHashPassword(password.trim());

    User.getOrCreateUser(firstName, lastName, email, userName, hashPassword, joinDate, memberId, nextSavingsDate, nextSavingsAmount, position, phone, status, isGuarantor).then(result => {
        const cleanedResult = {
            isNew: result.isNew,
            user: {
                id: result.user.id,
