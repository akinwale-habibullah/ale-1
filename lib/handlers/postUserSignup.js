 ...
 *   nextSavingsDate {integer} Next savings date of user to be registered.
 *   nextSavingsAmount {float} Next savings amount of user to be registered.
...
exports.handler = function postUserSignup(req, res, next) {
    const { firstName, lastName, email, userName, password, isAdmin, joinDate, memberId, nextSavingsDate, nextSavingsAmount, position, phone, status, isGuarantor, organization } = req.body;
    const hashPassword = generateHashPassword(password.trim());

    User.getOrCreateUser(firstName, lastName, email, userName, hashPassword, isAdmin, joinDate, memberId, nextSavingsDate, nextSavingsAmount, position, phone, status, isGuarantor, organization).then(result => {
        const cleanedResult = {
            isNew: result.isNew,
            user: {
                id: result.user.id,
